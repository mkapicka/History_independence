# 1. Setup
Load the calibration file and activate the project environment. Includes flow one direction (`solve ← model_settings ← run ← calibrate`), so this single `include` also defines `run_history_independent_tax` and the solver itself. Plotting is a separate layer: `include("plot_history_independent_tax.jl")` when needed.

**Start Julia with at least three threads.** The solver parallelizes over the `kappa` grid, so with the default `nKappa = 3` a single-threaded session takes about 2.6x as long (3.84s per solve against 1.46s here). Launch with `julia --project=. -t 3`, or set `JULIA_NUM_THREADS=3` in your shell profile; `julia.NumThreads` is the VS Code setting. More threads than `nKappa` buy nothing — 5 and 8 threads measure the same as 3.
...
Make sure you are in the current directory of the repository, or adjust the path to accordingly. The calibration file itself includes the model settings and solver, so you do not need to include those separately. 

```julia
include(raw"calibrate_history_independent_tax_Claude.jl")
...
## Result structure
`result` is a `NamedTuple`; every field is documented below. Throughout, `nAge = J + 1 = 40`: age-indexed vectors run over indices `1, …, J+1`, corresponding to economic ages `j = 0, …, J`.
...
**Note on moment (ii).** A borrowing limit exists only at ages `j = 0,…,J-1`. At the terminal age `j = J` the model imposes `a' ≥ 0` instead, so no limit is defined there. Both `meanBorrowingLimit` and `meanEffectiveGridBorrowingLimit` average `-bbar · exp(kappa + rho z)` over the mass of ages `j = 0,…,J-1`,
excluding the terminal age rather than counting it as a zero. Target (ii) is matched against that average, so `meanBorrowingLimitToMeanLaborIncome = 0.180` is the limit households actually face. The denominator is unchanged: mean labor income remains the whole-population mean over all `nAge = 40` ages.
...
The struct documented in the Section 2 table: the four targets, `asset_moment`, and the search controls, exactly as the run used them.

### `result.params` — the calibrated `HIParams`
Defaults in parentheses are the values in `SETTINGS` (`model_settings.jl`), which is the only place model parameters are defined; `HIParams` itself carries no defaults for them. Four fields (`maxIterLambda`, `massTol`, and the derived `omega_mean`/`epsilon_mean`/`kappa_mean`) are not in `SETTINGS` and default inside the struct.
| Field                        | Type                  | Meaning                                                  |
|------------------------------|-----------------------|----------------------------------------------------------|
...
The calibrated instruments `qSav`, `qBorr`, and `bbar` can be used to compute the equilibrium of the model. The equilibrium is computed by calling `run_history_independent_tax` with the calibrated parameters. The function returns a `NamedTuple` containing the equilibrium results, including the government budget residual, aggregate consumption, hours, labor income, assets by age, and other statistics.

```julia
include("run_history_independent_tax.jl")
result_run = run_history_independent_tax()
```

The function takes the following keyword arguments:
- `qSav`: calibrated saving price (e.g., from `result_calib.qSav`)
- `qBorr`: calibrated borrowing price (e.g., from `result_calib.qBorr`)
- `bbar`: calibrated borrowing limit (e.g., from `result_calib.bbar`)   
- other parameters that modify the baseline values in the `result_calib.params` struct.

## Result structure
`result` is a `NamedTuple`, a subset of result from `solve_history_independent_tax`; top fields are documented below.
...
# 5. Plotting
The plotting function `plot_history_independent_tax` takes the result of a run and produces a series of plots illustrating the equilibrium distributions, policy functions, and other relevant statistics. The function can be called as follows:
```julia
include("plot_history_independent_tax.jl")
plot_history_independent_tax(result_run)
```
...
## Calibration
Calibration with the default settings. Running the file as a script produces the same log and archives it in `calibration_results/` automatically.

```
julia --project=. -t 3
include("calibrate_history_independent_tax_Claude.jl")
result_calib = calibrate_history_independent_tax();
...
Options:
  ...
  qSav                        = 0.972347  (price of saving,    a' >= 0)
  qBorr                       = 1.012347  (price of borrowing, a' < 0)
  qGov                        = 0.990000  (government discount price)
  G                           = 0.000000  (government spending)
...
total solve time          = 1.477 seconds
...
model solves             = 144
calibration time         = 326.336 seconds
```

Note the `WARNING: upper bound is binding` in the solver log. At the calibrated point a mass share of `3.87e-7` sits exactly at `aMax = 15`, with zero slack. The share is far too small to move the targeted moments, but `aMax` is touching and should be raised before using this calibration for anything sensitive to theright tail of the asset distribution.


## Equilibrium computation
The calibrated instruments are used to compute the equilibrium of the model. The transcript below is reproducible: after the Section 1 setup it is the output of `run_history_independent_tax()`,

```
result_run = run_history_independent_tax(qSav = result_calib.qSav, qBorr = result_calib.qBorr, bbar = result_calib.bbar)
...
WARNING: upper bound is binding.
  asset upper bound       = BINDING (share = 3.86727009e-07, bound = 15.00000000, material max a' = 15.00000000, slack = 0.00000000e+00)
total solve time          = 1.568 seconds

=== Final history-independent equilibrium ===
lambda                     = 1.00207513
government budget residual = -6.19476248e-08
PV output                  = 30.34478945
PV consumption             = 30.34479565
mean output                = 0.91239803
mean consumption           = 0.91837067
terminal assets            = 0.00000000
solve time                 = 1.568 seconds

=== Aggregate statistics ===
mean assets / mean labor income          = 0.58750135
median assets / mean labor income        = 0.23948908
true borrowing limit / mean labor income = 0.18000194
grid borrowing limit / mean labor income = 0.17572545
share negative liquid assets             = 0.25993506
share at effective grid borrowing bound  = 0.11851803
share with zero assets                   = 0.02500000
share at upper asset bound               = 0.00000039
share at hours upper bound               = 0.00000000
...
## Plotting
The following command produces a series of plots illustrating the equilibrium distributions, policy functions, and other relevant statistics. The plots are saved in the `figures/` directory.

```
figs   = plot_history_independent_tax(result_run)

=== Figures saved ===
asset grid                = ~/Library/CloudStorage/Dropbox/Projects/HSV_HistoryDep_LifeCycle/code/julia/Bewley/hi/figures/asset_grid.png
average assets by age     = ~/Library/CloudStorage/Dropbox/Projects/HSV_HistoryDep_LifeCycle/code/julia/Bewley/hi/figures/average_assets_by_age.png
average hours by age      = ~/Library/CloudStorage/Dropbox/Projects/HSV_HistoryDep_LifeCycle/code/julia/Bewley/hi/figures/average_hours_by_age.png
assets                    = ~/Library/CloudStorage/Dropbox/Projects/HSV_HistoryDep_LifeCycle/code/julia/Bewley/hi/figures/unconditional_assets.png
hours worked              = ~/Library/CloudStorage/Dropbox/Projects/HSV_HistoryDep_LifeCycle/code/julia/Bewley/hi/figures/unconditional_hours_worked.png
consumption               = ~/Library/CloudStorage/Dropbox/Projects/HSV_HistoryDep_LifeCycle/code/julia/Bewley/hi/figures/unconditional_consumption.png
```